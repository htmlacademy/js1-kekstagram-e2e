Feature: 4. Просмотр загруженных изображений
  ...
  Scenario: 4.3. Загруженные изображения показаны в виде миниатюр
    ...
    Then миниатюра номер 1 содержит изображение 'photos/2.jpg'
    Then миниатюра номер 2 содержит альтернативный текст '#fun #party #cool #young'
    Then миниатюра номер 3 содержит 6 лайков
    Then миниатюра номер 4 содержит 6 комментариев

  Scenario: 4.4. При нажатии на любую из миниатюр, показывается блок .big-picture
    Given загружаю данные
    Given нахожусь на главной странице сайта
    When запрос завершён
    When кликаю на миниатюру номер 0
  ...
  Scenario: 4.4. При нажатии на любую из миниатюр, показывается блок .big-picture
    ...
    Given нахожусь на главной странице сайта
    When запрос завершён
    When кликаю на миниатюру номер 10
    Then элемент '.big-picture' видим
    Then элемент 'body' содержит класс 'modal-open'
  ...
  Scenario: 4.6. Сразу после открытия модального окна отображается не более 5 комментариев
    ...
    Then комментарий номер 1 содержит аватар 'img/avatar-2.svg'
    Then комментарий номер 2 содержит альтернативный текст 'Аким'
    Then комментарий номер 3 содержит текст 'Я залип на этой фотке и не могу оторваться. Совсем не знаю что мне делать. Разве это композиция?! Что это за композиция?!'
